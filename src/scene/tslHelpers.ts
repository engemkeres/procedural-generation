import {
    ...
    export const mixVec3 = Fn(({a, b, t}: {a: any, b:any, t:any}) => {
        return vec3(
          mix(a.r, b.r, t.r),
          mix(a.g, b.g, t.g),
          mix(a.b, b.b, t.b)
    ...
    export const stepVec2 = Fn(({filter, value}: {filter: any, value: any}) => {
        return vec2(
    ...
    export const plot = Fn(({ st, pct }: {st: any, pct: any}) => {
        return smoothstep(pct.sub(float(0.01)), pct, st.y).sub(smoothstep(pct, pct.add(float(0.01)), st.y))
    })

    export const smoothstepVec2 = Fn(({edge0, edge1, x}: {edge0: any, edge1: any, x:any}) => {
        return vec2(
    ...
    export const box = Fn(({st, size}: {st: any, size: any}) => {
    ...
    export const cross = Fn(({st, size}: {st:any, size:any}) => {
        return box({st, size: vec2(size, size.div(4.))}).add(
              box({st, size: vec2(size.div(4.), size)})
        )
    })

    export const rotate2D = Fn(({st, angle}: {st: any, angle:any}) => {
        st = st.sub(vec2(0.5))
        st = (mat2 as any)(
    ...
    export const scale2D = Fn(({scale}: {scale: any}) => {
        return (mat2 as any)( scale.x, 0.0,
                              0.0, scale.y
        )
    })

    export const tile2D = Fn(({st, x, y}: {st: any, x: any, y: any}) => {
        return fract(
          st.mul(vec2(x, y))
        )
    })

    export const brickTile = Fn(({st, zoom}: {st: any, zoom: any}) => {
        st = st.mul(zoom)
        st = st.add(vec2(
    ...
    export const rotateTilePattern = Fn(({st}: {st: any}) => {
        ...
        // if also works tho, should try it later on
    ...
    export const myRandom = Fn(({st}: {st: any}) => {
        return fract(
            sin(dot(st.xy, vec2(12.9898,78.233))).mul(43758.5453123)
        )
    })

    export const myRandom2 = Fn(({st}: {st: any}) => {
        st = vec2(
    ...
    export const truchetPattern = Fn(({st, index}: {st: any, index: any}) => {
    ...
    export const myFirstSmoothNoise = Fn(({x}: {x: any}) => {
        const i = floor(x)
        const f = fract(x)
        // fake hash random
    ...
    export const noise2D = Fn(({st}: {st: any}) => {
        const i = floor(st)
        const f = fract(st)
    ...
    export const gradientNoise = Fn(({st}: {st: any}) => {
        const i = floor(st)
        const f = fract(st)
    ...
    export const voronoi = Fn(({ st, uMouse, uOctaves }: { st: any, uMouse: any, uOctaves: any }) => {
        const points = array([
            vec2(0.83, 0.75),
            vec2(0.60, 0.07),
            vec2(0.28, 0.64),
    ...
    export const cellVoronoi = Fn(({st}: {st: any}) => {
        const iSt = floor(st)
    ...
    export const coloredVoronoi = Fn(({ st, uMouse, uOctaves }: { st: any, uMouse: any, uOctaves: any }) => {
        const point = array([
            vec2(0.83, 0.75),
            vec2(0.60, 0.07),
            vec2(0.28, 0.64),
    ...
    export const fbm = Fn(({st, uFrequency, uOctaves, uLacunarity, uGain, uTerrainMode}:
        ...
        const lacunarity = uLacunarity.toVar()
